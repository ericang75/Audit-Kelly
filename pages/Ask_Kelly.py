...
if "logged_in" not in st.session_state or not st.session_state["logged_in"]:
    st.warning("⚠️ You must login to access this page.")
    st.stop()
...
st.title("📘 NotebookLM — Audit")

# ---------------- Session defaults ----------------
# Inisialisasi default sesi
if "active_project" not in st.session_state:
    st.session_state.active_project = None
if "dfs" not in st.session_state:
    st.session_state.dfs = {}

# Menghancurkan cache saat berpindah proyek
proj = st.session_state.active_project
if proj not in st.session_state.dfs:
    st.session_state.dfs[proj] = {}

# Menjaga data proyek yang aktif dan menghapus data yang tidak terkait
for k in list(st.session_state["dfs"].keys()):
    if k != proj:
        st.session_state["dfs"].pop(k, None)
...
def project_path(name: str) -> str:
    """Return project path under the logged-in user's directory."""
    safe = safe_name(name)
    p = os.path.join(PROJECTS_DIR, safe)
...
def init_project_db(proj: str):
    ...
    """)
    # 🔹 Tambahkan tabel baru untuk hasil analisis AI
    c.execute("""
...
def get_previous_analyses(proj: str, limit: int = 5) -> List[Dict[str, str]]:
    """Ambil hasil analisis AI sebelumnya (limit default = 5)."""
    dbp = project_db_path(proj)
    conn = sqlite3.connect(dbp)
    c = conn.cursor()
    try:
        c.execute("SELECT ts, question, narrative FROM analysis_results ORDER BY id DESC LIMIT ?", (limit,))
        rows = c.fetchall()
        conn.close()
        return [{"ts": r[0], "question": r[1], "narrative": r[2]} for r in rows]
    except Exception:
        conn.close()
        return []
...
def generate_narrative(question: str, findings: Dict[str, Any], context_samples: Optional[Dict[str, Any]] = None) -> str:
    ...
    # 🔹 Ambil memory analisis lama
    previous_analyses = get_previous_analyses(proj, limit=3)
    context_memory = "\n\n".join(
        [f"[{a['ts']}] {a['question']}\n{a['narrative']}" for a in previous_analyses]
    )
    if not context_memory:
        context_memory = "(no prior analyses recorded)"
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Use prior analyses as context to maintain continuity and memory. "
                        "Your task: explain findings, highlight risks, "
                        "and suggest improvements clearly and concisely."
                    ),
                },
                {
                    "role": "user",
                    "content": 
                        f"Project Context (previous AI analyses):\n{context_memory}\n\n"
...
# ---------------- UI helpers ----------------
# Sidebar Panel - Memilih Proyek
def sidebar_panel():
    st.sidebar.header("📁 Projects")
    projects = list_projects()
    if "username" in st.session_state:
        st.sidebar.success(f"Hello, {st.session_state['first_name']} {st.session_state['last_name']}")
    ...
    if st.sidebar.button("🚪 Logout", type="primary"):
        from auth import logout_and_rerun
        logout_and_rerun()
